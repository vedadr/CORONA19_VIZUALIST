...
def calculate_change(current_data_holder, file, type, interval):
    ...
    # since the number of the counties changes on a daily level, we had to merge current with previous datasets
...
def calculate_cumulative_percent_change(current_data_holder, file, type, interval):
    ...
    # to make sure that proper counties are being subtracted
    temp_diff_holder = pd.merge(current_data_holder, previous_date_content, on='GeoID', how='left')
    return (temp_diff_holder[current_date_stamp + '_' + type] / temp_diff_holder[previous_period_stamp + '_' + type] * 100 - 100) \
...
def calculate_percent_change_period_over_period(current_data_holder, file, type, interval):
    ...
    temp_diff_holder = pd.merge(current_data_holder, previous_date_content, on='GeoID', how='left').merge(date_before_previous_date_content,
                                                                                                          on='GeoID', how='left')

    return (((temp_diff_holder[current_date_stamp + '_' + type]
              - temp_diff_holder[previous_date_stamp + '_' + type]) /
             (temp_diff_holder[previous_date_stamp + '_' + type]
              - temp_diff_holder[date_before_previous_day_stamp + '_' + type]))
            .replace([np.inf, -np.inf], np.nan) * 100 - 100).round(2)
...
def calculate_growth_rate(current_data_holder, file, type):
    ...
    return ((temp_diff_holder[current_date_stamp + '_' + type] - temp_diff_holder[previous_day_stamp + '_' + type]) / temp_diff_holder[
        previous_day_stamp + '_' + type] * 100) \
        .replace([np.inf, -np.inf], np.nan) \
        .round(1)
    ...
    for file in Path(path_to_preprocessed_files).iterdir():
        ...
        try:
            daily_data = daily_data \
                ...
                .assign(
                **{data_timestamp + '_percent_pop_confirmed_cases': lambda x: calculate_percent_pop(x, file, 'cases', config['acs_data_location'])}) \
                .assign(**{
                data_timestamp + '_daily_change_in_growth_confirmed_cases_percent': lambda x: calculate_percent_change_period_over_period(x, file,
                                                                                                                                          'cases',
                                                                                                                                          'day')}) \
                .assign(**{data_timestamp + '_daily_change_confirmed_cases_doubling_time': lambda x: calculate_doubling_rate(x, file, 'cases')}) \
                .assign(
                **{data_timestamp + '_confirmed_rate_per_100k': lambda x: calculate_rate_per_100k(x, file, 'cases', config['acs_data_location'])}) \
 \
                ...
                .assign(**{
                data_timestamp + '_daily_change_in_growth_deaths_percent': lambda x: calculate_percent_change_period_over_period(x, file, 'deaths',
                                                                                                                                 'day')}) \
                .assign(**{data_timestamp + '_daily_change_deaths_doubling_time': lambda x: calculate_doubling_rate(x, file, 'deaths')}) \
                .assign(
                **{data_timestamp + '_deaths_rate_per_100k': lambda x: calculate_rate_per_100k(x, file, 'deaths', config['acs_data_location'])}) \
 \
                .assign(**{data_timestamp + '_weekly_change_confirmed_cases': lambda x: calculate_change(x, file, 'cases', 7)}) \
                .assign(**{
                data_timestamp + '_weekly_change_in_growth_confirmed_cases_percent': lambda x: calculate_percent_change_period_over_period(x, file,
                                                                                                                                           'cases',
                                                                                                                                           'week')}) \
                .assign(**{
                data_timestamp + '_weekly_cumulative_change_in_confirmed_cases_percent': lambda x: calculate_cumulative_percent_change(x, file,
                                                                                                                                       'cases', 7)}) \
 \
                .assign(**{data_timestamp + '_weekly_change_deaths': lambda x: calculate_change(x, file, 'deaths', 7)}) \
                .assign(**{
                data_timestamp + '_weekly_change_in_growth_deaths_percent': lambda x: calculate_percent_change_period_over_period(x, file, 'deaths',
                                                                                                                                  'week')}) \
                .assign(**{
                data_timestamp + '_weekly_cumulative_change_in_deaths_percent': lambda x: calculate_cumulative_percent_change(x, file, 'deaths', 7)}) \
 \
                .assign(**{data_timestamp + '_monthly_change_confirmed_cases': lambda x: calculate_change(x, file, 'cases', 30)}) \
                .assign(**{
                data_timestamp + '_monthly_change_in_growth_confirmed_cases_percent': lambda x: calculate_percent_change_period_over_period(x, file,
                                                                                                                                            'cases',
                                                                                                                                            'month')}) \
                .assign(**{
                data_timestamp + '_monthly_cumulative_change_in_confirmed_cases_percent': lambda x: calculate_cumulative_percent_change(x, file,
                                                                                                                                        'cases', 30)}) \
 \
                .assign(**{data_timestamp + '_monthly_change_deaths': lambda x: calculate_change(x, file, 'deaths', 30)}) \
                .assign(**{
                data_timestamp + '_monthly_change_in_growth_deaths_percent': lambda x: calculate_percent_change_period_over_period(x, file, 'deaths',
                                                                                                                                   'month')}) \
                .assign(**{
                data_timestamp + '_monthly_cumulative_change_in_deaths_percent': lambda x: calculate_cumulative_percent_change(x, file, 'deaths',
                                                                                                                               30)}) \
 \
